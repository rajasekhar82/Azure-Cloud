...
echo "127.0.0.1      nomadx.sg" |sudo tee -a /etc/hosts
sudo mkdir -p /opt/nomadx/pub
sudo mkdir -p /opt/nomadx/var/log
sudo mkdir -p /opt/nomadx/generated
sudo mkdir /tmp/nomadx
sudo mount -t nfs -o,rw 10.0.0.11:/opt/nomadx  /tmp/nomadx
sudo rsync -avz --exclude="nomadx/pub" --exclude="var/log" --exclude="var/cache" --exclude="nomadx/generated" /tmp/nomadx/*   /opt/nomadx/
sudo mount -t nfs -o,rw 10.0.0.11:/home/atadmin/server-configuration/magento/nomadx/PROD  /tmp/serverconfig
sudo mount -t nfs -o,rw,nfsvers=3 10.0.0.11:/opt/nomadx/pub /opt/nomadx/pub
sudo mount -t nfs -o,rw,nfsvers=3 10.0.0.11:/opt/nomadx/var/log /opt/nomadx/var/log
...
sudo systemctl stop firewalld && sudo systemctl stop tuned
sudo systemctl disable firewalld && sudo systemctl disable tuned
sudo systemctl restart rsyslog && sudo systemctl enable crond
...
sudo echo "/usr/bin/mount -t nfs -o,rw,nfsvers=3 10.0.0.11:/opt/nomadx/pub /opt/nomadx/pub" >>/etc/rc.local
sudo echo "/usr/bin/mount -t nfs -o,rw,nfsvers=3 10.0.0.11:/opt/nomadx/var/log /opt/nomadx/var/log" >>/etc/rc.local
sudo umount -l /opt/nomadx/pub
sudo umount -l /opt/nomadx/log
sudo chmod +x /etc/rc.d/rc.local
sudo systemctl enable rc-local && sudo systemctl restart rc-local
##Nginx Amplify Agent
